...
public class SandboxMapas
{
    ...
    public List<String> getValoresComoLista( )
    {
        ...
        for (Map.Entry<String,String> parejas : mapaCadenas.entrySet())
        {
        	cadenas.add(parejas.getValue());
        }
        
        int actual = mapaCadenas.size();
        
        for (int i=0; i<actual -1; i++)
        {
            for(int j=1; j < actual - i; j++)
            {
                if (cadenas.get(j).compareTo(cadenas.get(j-1)) <0)
                {
                    String temp = cadenas.get(j);
                    cadenas.set(j, cadenas.get(j-1));
                    cadenas.set(j-1, temp);
                }
            }
        }
    ...
    public Collection<String> getLlaves( )
    {
        ...
        for (Map.Entry<String, String> parejas : mapaCadenas.entrySet())
        {
        	llaves.add(parejas.getKey().toUpperCase());
        }
    ...
    public int getCantidadCadenasDiferentes( )
    {
        ...
        for (Map.Entry<String, String> parejas : mapaCadenas.entrySet())
        {
        	if (!(unicos.contains(parejas.getValue())))
        	{
        		unicos.add(parejas.getValue());
        	}
        }
    ...
    public void volverMayusculas( )
    {
    	...
    	for (Map.Entry<String,String> parejas : mapaCadenas.entrySet())
    	{
    		String valor = parejas.getValue();
    		String llave = parejas.getKey();
    		
    		String nuevaLlave = llave.toUpperCase();
    		
    		nuevo.put(nuevaLlave, valor);
    	}
